...
<script>
import { getStudentLessonById,deleteStudentLesson } from "@/api/stules";
export default {
  methods: {
    deleteSCT(row) {
      const userinfo = JSON.parse(localStorage.getItem("userInfo"))
      const sid=userinfo["studentId"]
      const lid = row.lessonId
      const that = this
      deleteStudentLesson(sid,lid).then(function (resp) {
  ...
  created() {
    ...
    const sid=userinfo["studentId"]
    const that = this
    getStudentLessonById(sid).then(function (resp) {
